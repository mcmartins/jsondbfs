/**
 * (C) Copyright 2015 Manuel Martins.
 *
 * This module is inspired by json_file_system.
 * (json_file_system is Copyright (c) 2014 Jalal Hejazi,
 *  Licensed under the MIT license.)
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 *
 * Created by: ManuelMartins
 * Created on: 11-09-2015
 *
 */
 
'use strict';

require('babel/register');

var async = require('async'),
  _ = require('underscore'),
  criteriaTest = require('json-criteria').test,
  IOHandler = require('./iohandler');

/**
 *
 * @param options
 * @constructor
 */
function Collection(options) {
  options = options || {};
  this._db = options.db;
  this._dbFile = options.path;
  this._inMemory = options.inMemory;
  this._ioHandler = new IOHandler(options);
}

 
Collection.prototype.find = function(criteria, options, callback) {
  if (typeof options === 'function') {
    callback = options, options = {};
  }
  this._db._ioHandler.readFile(this._dbFile, function(err, documents) {
    if (err) {
      return callback(err);
    }
    if (!criteria) {
      return callback(null, documents);
    } else {
      var filteredDocuments = [];
      // filter documents using criteria in parallel
      async.each(documents, function filter(document, next) {
        if (criteriaTest(document, criteria)) {
          filteredDocuments.push(document);
          if (!options.multi) {
            // TODO go out if you just want to find just one record
          }
          next();
        }
      }, function finalize(err) {
        if (err) {
          return callback(err);
        } else {
          return callback(null, filteredDocuments);
        }
      });
    }
  });
};

Collection.prototype.findOne = function(criteria, callback) {
  if (!criteria) {
    return callback(new Error('No criteria specified!'));
  }
  this.find(criteria, {multi: false}, function(err, documents) {
    if (err) {
      return callback(err);
    }
    return callback(null, documents);
  });
};

Collection.prototype.findAndModify = function(criteria, updateCriteria, callback) {
  var self = this;
  if (!criteria) {
    return callback(new Error('No criteria specified!'));
  }
  if (!updateCriteria) {
    return callback(new Error('No update criteria specified!'));
  }
  self.find(criteria, function (err, documents) {
    if (err) {
      return callback(err);
    }
    if (documents) {
      // FIXME might lead to poor performance
      for (var document in documents) {
        for (var propertyName in updateCriteria) {
          if (updateCriteria.hasOwnProperty(propertyName) 
          && documents[document].hasOwnProperty(propertyName)) {
            documents[document][propertyName] = updateCriteria[propertyName];
          }
        }
        self.update({id: documents[document]._id}, document, callback)
      }
    }
  });
}

Collection.prototype.insert = function(data, callback) {
  var self = this;
  self._db._ioHandler.lockFile(function execute(err) {
    if (err) {
      // couldn't acquire the lock!
      return callback(err);
    }
    self._db._ioHandler.readFile(self._dbFile, function read(err, documents) {
      if (err) {
        // couldn't read the file!?
        return callback(err);
      }
      data._id = self._db._ioHandler.generateUUID();
      documents.push(data);
      self._db._ioHandler.writeFile(self._dbFile, documents, function finalize(err) {
        self._db._ioHandler.releaseLock();
        if (err) {
          // something happened while writing to the file!
          return callback(err);
        }
        return callback(null, data);
      });
    });
  });
};

Collection.prototype.update = function(criteria, data, options, callback) {
  if (typeof options === 'function') {
    callback = options, options = {};
  }
  var ret = {
    nMatched: 0,
    nModified: 0,
    nUpserted: 0
  };
  var self = this;
  self._db._ioHandler.lockFile(function execute(err) {
    if (err) {
      return callback(err);
    }
    self.find(criteria, function (err, filteredDocuments) {
      if (err) {
        return callback(err);
      }
      if (filteredDocuments.length) {
        ret.nMatched = filteredDocuments.length;
        if (err) {
           return callback(err);
         }
         if (options.multi) {
           for (var document in filteredDocuments) {
             filteredDocuments[document] = data;
             ret.updated++;
           }
         } else {
           filteredDocuments[0] = data;
           ret.nModified = 1;
         }
         self._db._ioHandler.readFile(self._dbFile, function read(err, documents) {
           if (err) {
             // couldn't read the file!?
             return callback(err);
           }
           var mergedDocuments = _.union(documents, filteredDocuments);
           self._db._ioHandler.writeFile(self._dbFile, mergedDocuments, function finalize(err) {
             self._db._ioHandler.releaseLock();
             if (err) {
               // something happened while writing to the file!
               return callback(err);
             }
             return callback(null, ret);
           });
         });
      } else {
        if (options.upsert) {
          self.insert(data, function handle(err){
            if (err) {
              // something happened with the insert
              return callback(err);
            }
            ret.nUpserted = 1;
            return callback(null, ret);
          });
        } else {
          return callback(null, ret);
        }
      }
    });
  });
};

Collection.prototype.remove = function(criteria, multi, callback) {
  if (!criteria) {
    return callback(new Error('No criteria specified!'));
  }
  var self = this;
  self._db._ioHandler.lockFile(function execute(err) {
    if (err) {
      return callback(err);
    }
    self._db._ioHandler.readFile(self._dbFile, function(err, documents) {
      if (err) {
        return callback(err);
      }
      var filteredDocuments = [];
      // filter collection using criteria in parallel
      async.each(documents, function filter(document, next) {
        if (criteriaTest(document, criteria)) {
          filteredDocuments.push(document);
        }
        next();
      }, function finalize(err) {
        if (err) {
          return callback(err);
        } else {
          self._db._ioHandler.writeFile(self._dbFile, filteredDocuments, function finalize(err) {
            self._db._ioHandler.releaseLock();
            if (err) {
              return callback(err);
            } else {
              return callback(null, true);
            }
          });
        }
      });
    });
  });
};

Collection.prototype.count = function(criteria, callback) {
  this._db._ioHandler.readFile(this._dbFile, function handle(err, documents) {
    return callback(err, documents.length);
  });
};

module.exports = Collection;