...
Collection.prototype.insert = function (data, callback) {
  if (!data) {
    throw new Error('No data passed to persist!');
  }
  // callback returns the data passed to this function,
  // if no callback provided we can proceed with a no callback default function
...
Collection.prototype.update = function (criteria, updateCriteria, options, callback) {
  ...
  var self = this;
  // callback returns the number of records updated or inserted,
  // if no callback provided we can proceed with a no callback default function
  callback = callback || noopCallback;
  self._db._ioHandler.lockFile(self._dbFileLock, function afterLockFile(err) {
    if (err) {
...
Collection.prototype.remove = function (criteria, options, callback) {
  ...
  var self = this;
  // callback returns true if objects are removed,
  // if no callback provided we can proceed with a no callback default function
  callback = callback || noopCallback;
  self._db._ioHandler.lockFile(self._dbFileLock, function afterLockFile(err) {
    if (err) {
